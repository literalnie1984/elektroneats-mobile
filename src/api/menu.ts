...
const API_URL = "https://9590-5-173-80-130.eu.ngrok.io"
...
export const getMenu = async function() {
		...
		week_days.map(async (week_day) => {
				fetched_values.push(fetch(`${API_URL}/api/menu/day/${week_day}`,
							{
								method: "GET",
								headers: {
								...
								if(response.ok) { console.log("Response ok"); return response.json(); } else { console.log("Response bruh"); return Promise.reject(null); }
						   }).then( data => {
								   console.log(data);
...
export const fetchMenu = async () => { return await getMenu(); };
