...
const CartItem = (props) => {
		...
		const optionsAnimatedStyle = useAnimatedStyle(() => ({ 
				height: (optionsHeight.value + "%"), display: optionsHeight.value === 0 ? "none" : 'flex', opacity: optionsHeight.value / 100,
		}));
		...
		return props.type === "meal" ? (
				<Animated.View style={[cartViewStyles.cartMeal, itemAnimatedStyle]}>
						<Pressable style={cartViewStyles.cartMealInfoBar} onPress={() => setIsExpanded(!isExpanded)}>
								<Text style={cartViewStyles.cartMealName}>
										Obiad 1
								</Text>
								<Text style={cartViewStyles.cartMealCost}>
										21.37zł
								</Text>
								<FontAwesomeIcon
										icon={isExpanded ? faChevronUp : faChevronDown}
										color={cartViewStyles.cartMealInfoIcon.color}
										size={cartViewStyles.cartMealInfoIcon.width}
								/>
						</Pressable>
						<Animated.View style={[cartViewStyles.cartMealActionsBar, optionsAnimatedStyle]}>
								...
								>
										<Text style={cartViewStyles.cartMealActionLabel}>
												Sprawdź/Zmień skład
										</Text>
								</Pressable>
